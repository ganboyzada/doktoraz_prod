...
@section('main')
<div class="md:h-[80vh]">
    <div class="grid lg:grid-cols-12 md:grid-rows-12 lg:grid-rows-1 gap-3 h-full">
        <div class="col-span-12 md:col-span-6 lg:col-span-6 xl:col-span-4 2xl:col-span-4 
                    md:row-span-8 lg:row-span-1
                    grid grid-cols-1 lg:grid-rows-12 gap-2 sm:gap-3">
            <div class="tile service-intro flex-col items-start p-6 row-span-5" style="--service-color: #3D4871;">
                
                <a href="{{ loc_route('news') }}" class="btn btn-adaptive mb-3">
                    <i data-feather="chevron-left"></i>
                    {{ s_trans('Xəbərlər') }}
                </a>
                <div class="post-published absolute right-8 top-8 flex gap-1 text-sm text-[#3D4871]">
            ...
            <div class="tile block row-span-7 p-0">
                <img class="h-full w-full object-cover" src="{{ media($blogpost->photo) }}" alt="">
            </div>
        </div>
        <div class="col-span-12 md:col-span-6 lg:col-span-6 xl:col-span-5 2xl:col-span-4 
                    md:row-span-8 lg:row-span-1
                    grid grid-cols-1 lg:grid-rows-12 gap-2 sm:gap-3">
        ...
        <div class="col-span-12 lg:col-span-12 xl:col-span-3 2xl:col-span-4 md:row-span-4 lg:row-span-1 tile flex-col py-6 has-read-more">
            <div class="mb-4 px-6">
                <h2 class="text-xl font-semibold">{{ s_trans('Digər xəbərlər') }}</h2>
            </div>
            <div class="service-tiles grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-1 2xl:grid-cols-1 gap-2 overflow-y-scroll pb-32 px-6">
                @foreach ($related_posts as $rpost)
                <div class="blogpost-wrapper">
                    <a href="{{ loc_route('news.find', $rpost->slug) }}">
                        <div class="blogpost grid grid-cols-12 radius-x overflow-hidden tile h-full">
                            <img class="col-span-4 h-36 min-h-full w-full object-cover" src="{{ media($rpost->photo) }}" alt="">
                            
                            <div class="col-span-8 post-details px-4 py-3 flex flex-col items-start gap-2">
